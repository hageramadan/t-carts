...
export function StoreFeaturesSection() {
  ...
  // تحديد التصميم لكل عنصر حسب الترتيب (نفس تقسيمة الكاتيجوري)
  const getFeatureLayout = (index: number) => {
    if (index === 0) return { colSpan: "lg:col-span-1", rowSpan: "", height: "h-[400px]" };
    if (index === 1) return { colSpan: "lg:col-span-2", rowSpan: "row-span-1", height: "h-[400px]" };
    if (index === 2) return { colSpan: "lg:col-span-1", rowSpan: "row-span-1", height: "h-[400px]" };
    if (index === 3) return { colSpan: "lg:col-span-2", rowSpan: "", height: "h-[400px]" };
    if (index === 4) return { colSpan: "lg:col-span-2", rowSpan: "", height: "h-[400px]" };
    return { colSpan: "lg:col-span-1", rowSpan: "", height: "h-[400px]" };
  };
  ...
  return (
    <section className="py-8 md:py-16 container mx-auto px-8" dir={language === "ar" ? "rtl" : "ltr"}>
      ...
      {/* شبكة المميزات بنفس تقسيمة الكاتيجوري */}
      <motion.div
        ...
        {featuresData.map((feature, index) => {
          ...
          return (
            <motion.div
              ...
              className={`${layout.colSpan} ${layout.rowSpan} ${layout.height} group`}
            >
              ...
              <div className="relative bg-[#4DA475] h-full w-full overflow-hidden rounded-2xl shadow-md hover:shadow-2xl transition-all duration-500 hover:-translate-y-3">
                ...
                <Image
                  ...
                  priority={index < 2}
                  style={{ objectPosition: 'center center', top: '8px' }}
                />
